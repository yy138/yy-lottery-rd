...
def parse_one_page(html):  
    ...
    i = 0  
    for item in soup.select('tr')[5:-8]:  
 
        yield{  
            #'time':item.select('td')[i].text,  
            'qihao':item.select('td')[i].text,  
            'qianqu':item.select('td')[i+1].text,  
...
def write_to_excel():  
    ...
    for k in range(2007,t_year+1):  
        #url = 'http://kaijiang.zhcw.com/zhcw/html/3d/list_%s.html' %(str(k))  
        #if k<2019:      
        url = 'http://www.ffcp.cn/zs/DLT/%s.html' %('DLT') if k == t_year else  'http://www.ffcp.cn/zs/DLT/%s.html' %('DLTY'+str(k))
         #   pass
        #else:
        ...
        for item in parse_one_page(html):  
            ...
            sheet1.write(i+1,1,item['qianqu'])  
            sheet1.write(i+1,2,item['houqu'])  
